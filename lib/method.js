...
Meteor.methods({
  ...
  updateMyProfile(profile) {
    ...
    for(var key in newProfile){
      if(!newProfile[key]) delete newProfile[key];
    }
    Meteor.users.update(
      Meteor.userId(),
      {$set: newProfile});
  ...
  changeRating(userId, rating) {
    ...
    Reviews.update(
      reviewId,
      {$set: {detail: rating.detail, rate: rating.rate}});
    if (Meteor.user().score <=0 || rating.rate == oldRate) {
    ...
    Meteor.users.update(userId, {$set: {score: reviewee.score + scoreInc}});
  },
  ...
  createRoom (userId) {
    ...
    var roomId = Rooms.insert({
      ...
      lastUpdated: Date.now(),
      lastUpdatedUser: this.userId,
      title: Meteor.user().username + '和' + theOtherUser.username + '的聊天室',
      creater: this.userId,
    });
